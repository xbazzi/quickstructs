...
// C++20 for C++
// /////////////////////////////////////////////////////////////////////////
// If C++: Your code is automatically compiled with a
// precompiled header. // 99% of includes / packages are
// already added for you.                  // You do NOT
// need to add your own includes here. //
// /////////////////////////////////////////////////////////////////////////

// C++23 - Orderbook example with a ranges-based main
// Cleaned up to use modern ranges idioms in `main` and
// fixed stray syntax.
   ...
   public:
    ...
    Trades AddOrder(const Order& incoming) {
        ...
        while (!opposite_side.empty() and remaining > 0)
        {
            ...
            Id bid_order_id =
                incoming.is_buy() ? incoming.get_id() : best.get_id();
            Id ask_order_id =
                incoming.is_buy() ? best.get_id() : incoming.get_id();

            // send trade since there is a match
            trades.emplace_back(Trade{ bid_order_id, ask_order_id, aggressor_id,
                                       aggressor_is_buy, trade_price,
                                       trade_size });
...
int main() {
    ...
    std::ranges::generate_n(std::back_inserter(orders), n_orders,
                            [&]() mutable
                            {
                                bool currentisbuy = isbuy;
                                isbuy             = !isbuy;
                                return Order{ ++id, price++, currentisbuy,
                                              qty++ };
                            });
