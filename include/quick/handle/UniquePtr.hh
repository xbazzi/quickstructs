...
template <typename T, typename Deleter = default_deleter<T>> class UniquePtr
{
  private:
    // Arrays are not supported in this implementation
    static_assert(!std::is_array_v<T>, "Array types not supported. Use UniquePtr<T[]> specialization.");
  ...
  public:
    ...
    UniquePtr(Pointer_T ptr, const Deleter &del) noexcept(std::is_nothrow_copy_constructible_v<Deleter>)
    ...
    UniquePtr(Pointer_T ptr, Deleter &&del) noexcept(std::is_nothrow_move_constructible_v<Deleter>)
    ...
    UniquePtr(UniquePtr &&other) noexcept : m_ptr(other.m_ptr), m_deleter(std::forward<Deleter>(other.m_deleter))
    {
        other.m_ptr = nullptr;
    }
    ...
    template <typename U, typename D,
              typename = std::enable_if_t<std::is_convertible_v<typename UniquePtr<U, D>::Pointer_T, Pointer_T> &&
                                          !std::is_array_v<U>>>
...
template <typename T, typename D> void swap(UniquePtr<T, D> &lhs, UniquePtr<T, D> &rhs) noexcept
{
    lhs.swap(rhs);
}
...
[[nodiscard]] bool operator==(const UniquePtr<T1, D1> &lhs, const UniquePtr<T2, D2> &rhs) noexcept
{
...
[[nodiscard]] bool operator<(const UniquePtr<T1, D1> &lhs, const UniquePtr<T2, D2> &rhs) noexcept
{
    using CT = std::common_type_t<typename UniquePtr<T1, D1>::Pointer_T, typename UniquePtr<T2, D2>::Pointer_T>;
...
template <typename T, typename D> [[nodiscard]] bool operator==(const UniquePtr<T, D> &ptr, std::nullptr_t) noexcept
{
    return !ptr;
}

template <typename T, typename D> [[nodiscard]] bool operator==(std::nullptr_t, const UniquePtr<T, D> &ptr) noexcept
{
    return !ptr;
}
